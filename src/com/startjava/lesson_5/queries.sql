...
SELECT * FROM Jaegers WHERE status = 'Active' ORDER BY model_name;
...
SELECT * FROM Jaegers WHERE launch < (SELECT AVG(launch) FROM Jaegers) ORDER BY launch;

-- Информация из столбцов model_name, mark, launch, kaiju_kill тех роботов, которые уничтожили больше всех kaiju.
SELECT model_name, mark, launch, kaiju_kill FROM Jaegers WHERE kaiju_kill >= (SELECT AVG(kaiju_kill) FROM Jaegers) ORDER BY kaiju_kill ;

-- Cредний вес роботов, округленный до трех знаков после запятой.
SELECT ROUND((AVG(weight)) :: numeric, 3) FROM Jaegers;
